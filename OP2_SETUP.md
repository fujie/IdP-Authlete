...
- **Service ID**: [OP2_SERVICE_ID]（`.env.op2`ファイルを参照）
...
⚠️ **JWK Set設定が必須**: OP2のAuthlete Serviceには、JWK Set（JSON Web Key Set）の設定が必要です。設定されていない場合、JWT署名検証が失敗し、認証フローが完了しません。
...
1. OP2の**Service ID**（`.env.op2`ファイルを参照）にアクセス
...
2. **Select OP**セクションで`https://op2.example.com`を入力
3. **Select OP**をクリック
4. ✅ OP2が選択されたことを確認
...
**症状**: 
```
Error: Authlete Service is missing JWK Set configuration
```
...
**解決方法**:
1. Authlete管理画面にアクセス
2. OP2のService IDを選択（`.env.op2`ファイルを参照）
...
**最終更新**: 2026年2月4日  
**OP2 Entity ID**: https://op2.example.com  
**OP2 Port**: 3002  
**Authlete Service ID**: `.env.op2`ファイルを参照
